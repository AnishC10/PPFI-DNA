...
class FHEModel:
    def __init__(self, input_size=2292, hidden_size=64, output_size=2, n_layers=2,
                 n_w_bits=8, maxiter=200, activation='relu', verbose=1, batch_size=256):
    ...
    def predict(self, X, encrypted=False):
        ...
        return self.model.predict(X, fhe="execute" if encrypted else "disable")
...
class FederatedLearning:
    def __init__(self, num_clients, model_class, input_size=2292, hidden_size=64, output_size=2,
                 n_layers=2, n_w_bits=8, maxiter=100, activation='relu', verbose=0,
                 global_epochs=5, local_epochs=1, alpha=0.01, batch_size=256):
    ...
    def train(self, X, y, split_method="horizontal", non_iid_factor=0,
        ...
        # Initialize with more samples
        sample_X = X[:min(5000, len(X))]
        sample_y = y[:min(5000, len(y))]
        ...
        for epoch in range(self.global_epochs):
            ...
            if X_test is not None and y_test is not None:
                temp_preds = self.global_model.model.predict(X_test, fhe="disable")
                acc = np.mean(temp_preds == y_test)
                f1 = f1_score(y_test, temp_preds)
                metrics_history.append({"epoch": epoch+1, "accuracy": acc, "f1": f1})
                if self.verbose > 0:
        ...
        if compile_for_fhe:
            ...
            if X_test is not None and y_test is not None:
                ...
                if self.verbose > 0:
                    ...
                    print(f"F1 Score: {f1_score(y_test[:test_subset], encrypted_preds):.4f}")
...
if __name__ == "__main__":
    def preprocess_data(csv_path):
        encodings = {'A': [1, 0, 0, 0], 'T': [0, 1, 0, 0], 'C': [0, 0, 1, 0], 'G': [0, 0, 0, 1], 'N': [0, 0, 0, 0]}
        ...
        X = data['sequence']
        X_processed = np.array([encode(nucleotide) for nucleotide in X]).reshape(len(X), -1)
        y_processed = data['label'].values
    ...
    X_train, y_train = preprocess_data("data_padded.csv")
    X_test, y_test = preprocess_data("test_data_padded.csv")
    ...
    start_time = time.time()
    encrypted_predictions = final_model.predict(X_test[:100], encrypted=True)
    end_time = time.time()
    encrypted_accuracy = np.mean(encrypted_predictions == y_test[:100])
    print(f"\nEncrypted Test Accuracy (100 samples): {encrypted_accuracy:.4f}")
    print(f"Encrypted Test F1 (100 samples): {f1_score(y_test[:100], encrypted_predictions):.4f}")
    print(f"Encrypted runtime: {end_time-start_time:.2f} seconds")
